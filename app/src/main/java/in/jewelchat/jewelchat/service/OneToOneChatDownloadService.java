			...
			for(int i=0; i<chats.length(); i++){
				//process messages
					...
					switch(eventname){
						...
						case "publish_ack":{
							Intent s = new Intent(getApplicationContext(), UpdatePublishAck.class);
							s.putExtra("json", packet.toString());
							startService(s);
							break;
						}
						case "msg_delivery":{
							Intent s = new Intent(getApplicationContext(), UpdateMessageDelivered.class);
							s.putExtra("json", packet.toString());
							startService(s);
							break;
						}
						case "msg_read":{
							Intent s = new Intent(getApplicationContext(), UpdateMessageRead.class);
							s.putExtra("json", packet.toString());
							startService(s);
							break;
						}
